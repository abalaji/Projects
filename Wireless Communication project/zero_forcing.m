...
for i = 1:length(Eb_N0_dB)
    ...
    %Formation of the Zero Forcing equalization matrix W=inv(H^H*H)*H^H
    ...
    Den_h = reshape(kron(reshape(Den_h,1,N/Tr),ones(2,2)),2,2,N/Tr); 
    Inverse_h = Coeff_h./Den_h; %evaluating inverse
    Modu_h =  reshape(conj(h),Rcvr,N); %H^H (hermitian conjugate
    
    Modu_out = kron(y,ones(1,2)); %received symbol formatted
    Modu_out = sum(Modu_h.*Modu_out,1); %H^H*y 
    Modu_out =  kron(reshape(Modu_out,2,N/Tr),ones(1,2)); 
...
axis([0 25 10^-5 0.5])
grid on
legend('theoretical (Tr=1,Rcvr=1)', 'theoretical (Tr=1,Rcvr=2, MRC)', 'simulation (Tr=2, Rcvr=2, ZF)');
xlabel('Average Eb/No in dB');
ylabel('BER');
title('BER for ZF equalizer ');